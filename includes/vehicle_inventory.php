        ...
        foreach ($rows as $row) {
            ...
            if ($showAllWarehouses && $productId > 0) {
                    ...
                    if ($warehouseType === 'vehicle' && !empty($warehouseVehicleId)) {
                        ...
                        if ($productInWarehouse) {
                            ...
                            if ($warehouseQuantity <= 0) {
                                ...
                                if ($warehouseQuantity > 0 && $batchId > 0) {
                ...
                if ($actualQuantity === null || $actualQuantity <= 0) {
                    ...
                    // خصم الكميات المنقولة بالفعل (approved أو completed) إذا كان fromWarehouseId موجوداً
                    if ($fromWarehouseId && $batchId > 0) {
                        $transferred = $db->queryOne(
                            "SELECT COALESCE(SUM(
                                CASE
                                    WHEN wt.status IN ('approved', 'completed') THEN wti.quantity
                        ...
                        // إذا لم يكن هناك fromWarehouseId، نخصم جميع النقلات المعتمدة
                        $transferred = $db->queryOne(
                            "SELECT COALESCE(SUM(
                                CASE
                                    WHEN wt.status IN ('approved', 'completed') THEN wti.quantity
        ...
        foreach ($items as $item) {
            ...
            if ($batchId) {
                ...
                if ($isFromVehicleWarehouse && $fromVehicleId) {
                    ...
                    // ملاحظة: quantity_produced يتم تحديثه عند النقل (يتم خصم الكمية المنقولة منه)
                    // لذلك لا نحتاج لخصم الكميات المنقولة (approved أو completed) مرة أخرى
        ...
        foreach ($items as $item) {
            ...
            if ($batchId) {
                ...
                if (($fromWarehouse['warehouse_type'] ?? '') === 'vehicle' && !empty($fromWarehouse['vehicle_id'])) {
                    ...
                    if ($productId > 0) {
                        if ($batchId) {
                            ...
                            if ($vehicleInventoryCheck) {
                                ...
                                $actualBatchId = (int)($vehicleInventoryCheck['finished_batch_id'] ?? $batchId);
                            } else {
                                // إذا لم نجد سجل مع finished_batch_id = batchId، نبحث عن جميع السجلات بنفس product_id
                                // ونستخدم finished_batch_id من السجل الذي يحتوي على كمية
                                $allVehicleInventory = $db->query(
                                    "SELECT quantity, finished_batch_id FROM vehicle_inventory 
                                     WHERE vehicle_id = ? AND product_id = ? 
                                ...
                                if (!empty($allVehicleInventory)) {
                                    // استخدام السجل الأول (الأكبر كمية)
                                    $vehicleInventoryCheck = $allVehicleInventory[0];
                                    $availableQuantity = (float)($vehicleInventoryCheck['quantity'] ?? 0);
                                    $actualBatchId = (int)($vehicleInventoryCheck['finished_batch_id'] ?? $batchId);
                            ...
                            // إذا لم يكن هناك batch_id، نجمع الكميات من جميع السجلات التي لا تحتوي على finished_batch_id
                            $vehicleInventorySum = $db->queryOne(
                            ...
                            $availableQuantity = (float)($vehicleInventorySum['total_quantity'] ?? 0);
                        }
                        
                        // خصم الكمية المحجوزة في طلبات النقل المعلقة (pending) من نفس المخزن
                        if ($actualBatchId && $productId > 0) {
                            $pendingTransfers = $db->queryOne(
                                "SELECT COALESCE(SUM(wti.quantity), 0) AS pending_quantity
                                 FROM warehouse_transfer_items wti
                                 INNER JOIN warehouse_transfers wt ON wt.id = wti.transfer_id
                                 WHERE wti.batch_id = ? 
                                   ...
                                   AND wt.id != ?",
                                [$actualBatchId, $productId, $transfer['from_warehouse_id'], $transferId]
                            );
                            $availableQuantity -= (float)($pendingTransfers['pending_quantity'] ?? 0);
                        }
                    ...
                    if ($batchRow) {
                        // quantity_produced يحتوي بالفعل على الكمية المتبقية بعد خصم النقل السابق
                        $availableQuantity = (float)($batchRow['quantity_produced'] ?? 0);
                ...
                // إذا كان المخزن المصدر سيارة، نفحص مخزون السيارة
                if (($fromWarehouse['warehouse_type'] ?? '') === 'vehicle' && !empty($fromWarehouse['vehicle_id'])) {
                    // جمع الكميات من جميع السجلات التي لها نفس product_id و finished_batch_id = NULL أو 0
                    // (للمنتجات التي لا تحتوي على batch_id)
                    $fromVehicleStockRow = $db->queryOne(
                        "SELECT COALESCE(SUM(quantity), 0) AS total_quantity 
                         FROM vehicle_inventory 
                         WHERE vehicle_id = ? 
                    ...
                    if (!$fromVehicleStockRow || (float)($fromVehicleStockRow['total_quantity'] ?? 0) <= 0) {
                        error_log("Product not found in vehicle inventory - Vehicle ID: {$fromWarehouse['vehicle_id']}, Product ID: $productId");
                        $availableQuantity = 0.0;
                    } else {
                        $availableQuantity = (float)($fromVehicleStockRow['total_quantity'] ?? 0);
            ...
            if ($toWarehouse && ($toWarehouse['warehouse_type'] ?? '') === 'vehicle' && !empty($toWarehouse['vehicle_id'])) {
                ...
                if ($batchId && ($toWarehouse['warehouse_type'] ?? '') === 'main') {
                    ...
                    if ($existingFinishedProd) {
                        ...
                        error_log("approveWarehouseTransfer: Updated finished_products.quantity_produced for batch_id=$batchId: $currentQtyProduced + $requestedQuantity = $newQtyProduced (transfer from vehicle to main warehouse)");
                    } else {
