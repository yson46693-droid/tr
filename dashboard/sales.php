...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_cash_balance') {
    ...
    if ($pageParam === 'cash_register') {
        // التحقق من طلب AJAX - إذا كان AJAX، اترك cash_register.php يتعامل معه
        $isAjax = !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest';
        
        // إذا كان طلب AJAX، لا نعالجه هنا ونترك cash_register.php يتعامل معه
        // لأن cash_register.php لديه معالجة AJAX كاملة
        if ($isAjax) {
            // لا نفعل أي شيء هنا - سيتم معالجته في cash_register.php
        } else {
            // للطلبات العادية (غير AJAX)، نعالجها هنا ونعيد التوجيه
            // تنظيف أي output buffer
            while (ob_get_level() > 0) {
                ob_end_clean();
            }
            ...
            // إعادة التوجيه لتجنب إعادة إرسال النموذج (للطلبات العادية فقط)
            if (!empty($success)) {
...
$pageParam = $_GET['page'] ?? 'dashboard';
$isCashRegisterAjax = $isAjaxRequest && $pageParam === 'cash_register' && isset($_POST['action']) && $_POST['action'] === 'add_cash_balance';

// إذا كان طلب AJAX لصفحة cash_register، ندرج الملف مباشرة ونخرج (قبل أي output HTML)
if ($isCashRegisterAjax) {
    $modulePath = __DIR__ . '/../modules/sales/cash_register.php';
    if (file_exists($modulePath)) {
        include $modulePath;
    } else {
        // في حالة عدم وجود الملف، نرسل استجابة JSON خطأ
        header('Content-Type: application/json; charset=utf-8');
        echo json_encode([
            'success' => false,
            'message' => 'صفحة خزنة المندوب غير متاحة'
        ], JSON_UNESCAPED_UNICODE);
    }
    exit;
}
