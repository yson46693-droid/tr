...
// تسجيل جميع المعلومات للتحقق - معطل
// $debugFile = __DIR__ . '/../debug_check.log';
// file_put_contents($debugFile, "=== REQUEST CHECK ===\n[" . date('d-M-Y H:i:s T') . "]\n", FILE_APPEND);
// file_put_contents($debugFile, "REQUEST_METHOD: " . ($_SERVER['REQUEST_METHOD'] ?? 'NOT SET') . "\n", FILE_APPEND);
// file_put_contents($debugFile, "GET page: " . ($_GET['page'] ?? 'NOT SET') . "\n", FILE_APPEND);
// file_put_contents($debugFile, "POST action: " . ($_POST['action'] ?? 'NOT SET') . "\n", FILE_APPEND);
...
// file_put_contents($debugFile, "Page variable: $page\n", FILE_APPEND);
// file_put_contents($debugFile, "Condition check - page === 'representatives_customers': " . ($page === 'representatives_customers' ? 'TRUE' : 'FALSE') . "\n", FILE_APPEND);
// file_put_contents($debugFile, "Condition check - REQUEST_METHOD === 'POST': " . ($_SERVER['REQUEST_METHOD'] === 'POST' ? 'TRUE' : 'FALSE') . "\n", FILE_APPEND);
// file_put_contents($debugFile, "Condition check - isset(POST[action]): " . (isset($_POST['action']) ? 'TRUE' : 'FALSE') . "\n", FILE_APPEND);
// file_put_contents($debugFile, "Condition check - POST[action] === 'collect_debt': " . (($_POST['action'] ?? '') === 'collect_debt' ? 'TRUE' : 'FALSE') . "\n", FILE_APPEND);
...
if ($page === 'representatives_customers' && 
    ...
    // file_put_contents($debugFile, "CONDITION PASSED - Starting collection processing\n", FILE_APPEND);
...
// تسجيل جميع الطلبات للتحقق - معطل
...
// تسجيل جميع الطلبات POST للتحقق - معطل
...
// تم نقل معالجة POST إلى البداية المطلقة (السطر 17-99)
...
// معالجة AJAX لصفحة company_cash (رصيد المندوب أو جدول المعاملات بدون ريفريش)
if ($page === 'company_cash' && isset($_GET['ajax'])) {
    $ajaxAction = $_GET['ajax'];
    if ($ajaxAction === 'get_sales_rep_balance' || $ajaxAction === 'transactions_list') {
        // تحميل الملفات الأساسية فقط قبل أي إخراج
        require_once __DIR__ . '/../includes/config.php';
        require_once __DIR__ . '/../includes/db.php';
        require_once __DIR__ . '/../includes/auth.php';
        require_once __DIR__ . '/../includes/audit_log.php';
...
<?php endif; ?>
            ...
            <?php if ($page === 'overview' || $page === ''): ?>
                ...
                <div class="cards-grid mt-4">
                    ...
                    $balance = $approvedIncome - $approvedExpense - $approvedPayment - $totalSalaries - $totalSalaryAdjustments - $totalCustomerCreditSettlements - $totalManagementSupplies;
            ...
            <?php elseif ($page === 'regions'): ?>
                <!-- صفحة إدارة المناطق -->
                <?php
                $modulePath = __DIR__ . '/../modules/manager/regions.php';
                if (file_exists($modulePath)) {
                    try {
                        include $modulePath;
                    } catch (Throwable $e) {
                        error_log('Regions module error: ' . $e->getMessage());
                        echo '<div class="alert alert-danger">حدث خطأ أثناء تحميل صفحة إدارة المناطق: ' . htmlspecialchars($e->getMessage()) . '</div>';
                    }
                } else {
                    echo '<div class="alert alert-warning">صفحة إدارة المناطق غير متاحة حالياً</div>';
                }
                ?>
            ...
            <?php elseif ($page === 'returns' || $page === 'returns_overview'): ?>
                ...
                if (file_exists($modulePath)) {
                    try {
                        ...
                        error_log('Manager returns module error: ' . $e->getMessage());
            ...
            <div id="${toastId}" class="toast align-items-center text-white bg-success border-0" role="alert" aria-live="assertive" aria-atomic="true" data-bs-autohide="true" data-bs-delay="5000">
                <div class="d-flex">
                    <div class="toast-body">
                        ...
                        <div class="small">${message.replace(/\n/g, '<br>')}</div>
                    </div>
...
function approveRequest(id, event) {
    ...
    .then(data => {
        if (data.success) {
            // إظهار رسالة النجاح
            showSuccessToast('تمت الموافقة بنجاح!', data.message || 'تمت الموافقة على الطلب بنجاح.');
            ...
            if (row) {
                ...
                setTimeout(() => {
                    ...
                    if (tbody && tbody.querySelectorAll('tr').length === 0) {
                        tbody.innerHTML = '<tr><td colspan="5" class="text-center text-muted">لا توجد موافقات معلقة</td></tr>';
                    }
                }, 500);
            }
...
function rejectRequest(id, evt) {
    ...
    .then(data => {
        if (data.success) {
            // إظهار رسالة النجاح
            showSuccessToast('تم الرفض بنجاح!', data.message || 'تم رفض الطلب بنجاح.');
            ...
            // إزالة الصف من الجدول بعد الرفض (بدون إعادة تحميل فورية)
            const row = btn ? btn.closest('tr') : null;
            if (row) {
                row.style.transition = 'opacity 0.5s';
                row.style.opacity = '0';
