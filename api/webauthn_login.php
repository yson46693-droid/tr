...
try {
    if ($action === 'create_challenge') {
    ...
    } elseif ($action === 'verify_stored_accounts') {
        // التحقق من أن الحسابات المحفوظة محلياً ما زالت نشطة ولديها بصمات
        $accountIdsJson = $_POST['account_ids'] ?? '[]';
        $accountIds = json_decode($accountIdsJson, true);
        
        if (!is_array($accountIds) || empty($accountIds)) {
            http_response_code(400);
            echo json_encode(['success' => false, 'error' => 'معرفات الحسابات مطلوبة'], JSON_UNESCAPED_UNICODE);
            exit;
        }
        ...
        try {
            ...
            // جلب الحسابات النشطة التي لديها بصمات مسجلة
            $users = $db->query(
                "SELECT DISTINCT u.id as user_id, u.username, u.role, u.full_name, u.status,
                        (SELECT COUNT(*) FROM webauthn_credentials wc WHERE wc.user_id = u.id) as credentials_count
                 FROM users u
            ...
            error_log("Verify stored accounts error: " . $e->getMessage());
            http_response_code(500);
            echo json_encode(['success' => false, 'error' => 'حدث خطأ أثناء التحقق من الحسابات'], JSON_UNESCAPED_UNICODE);
        }
