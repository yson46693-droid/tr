...
// معالجة حذف العميل قبل أي شيء آخر
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && trim($_POST['action']) === 'delete_company_customer') {
    // التحقق من الصلاحيات - فقط المدير يمكنه حذف العملاء
    $currentRole = strtolower((string)($currentUser['role'] ?? ''));
    if ($currentRole !== 'manager') {
        $_SESSION['error_message'] = 'غير مصرح لك بحذف العملاء. هذه الصلاحية متاحة للمدير فقط.';
        $section = $_GET['section'] ?? ($_POST['section'] ?? 'company');
        redirectAfterPost('customers', ['section' => $section], [], $currentRole);
        exit;
    }
    ...
    try {
        ...
        try {
            // 1. حذف return_items المرتبطة بـ returns الخاصة بهذا العميل
            $returnsIds = $db->query("SELECT id FROM returns WHERE customer_id = ?", [$customerId]);
            if (!empty($returnsIds)) {
                $returnIdsArray = array_column($returnsIds, 'id');
                if (!empty($returnIdsArray)) {
                    $placeholders = implode(',', array_fill(0, count($returnIdsArray), '?'));
                    $db->execute("DELETE FROM return_items WHERE return_id IN ($placeholders)", $returnIdsArray);
                }
            }
            
            // 2. حذف returns الخاصة بالعميل
            $db->execute("DELETE FROM returns WHERE customer_id = ?", [$customerId]);
            
            // 3. حذف invoice_items المرتبطة بـ invoices الخاصة بهذا العميل
            $invoicesIds = $db->query("SELECT id FROM invoices WHERE customer_id = ?", [$customerId]);
            if (!empty($invoicesIds)) {
                ...
                if (!empty($invoiceIdsArray)) {
                    ...
                    $db->execute("DELETE FROM invoice_items WHERE invoice_id IN ($placeholders)", $invoiceIdsArray);
                }
            }
            
            // 4. حذف invoices الخاصة بالعميل
            $db->execute("DELETE FROM invoices WHERE customer_id = ?", [$customerId]);
            
            // 5. حذف collections الخاصة بالعميل
            $db->execute("DELETE FROM collections WHERE customer_id = ?", [$customerId]);
            
            // 6. حذف customer_purchase_history الخاصة بالعميل
            $db->execute("DELETE FROM customer_purchase_history WHERE customer_id = ?", [$customerId]);
            
            // 7. حذف sales الخاصة بالعميل
            $db->execute("DELETE FROM sales WHERE customer_id = ?", [$customerId]);
            
            // 8. حذف customer_phones الخاصة بالعميل
            ...
            $_SESSION['success_message'] = 'تم حذف العميل "' . htmlspecialchars($customerName) . '" وجميع السجلات المرتبطة به بنجاح.';
            
        } catch (Throwable $deleteError) {
            // إلغاء المعاملة في حالة الخطأ
            $db->rollback();
            error_log('Delete customer error: ' . $deleteError->getMessage());
            error_log('Stack trace: ' . $deleteError->getTraceAsString());
            throw $deleteError;
        }
        
    } catch (Throwable $e) {
        error_log('Delete customer transaction error: ' . $e->getMessage());
        $_SESSION['error_message'] = 'حدث خطأ أثناء حذف العميل: ' . $e->getMessage();
    }
        ...
        addRegionFromCustomerForm.addEventListener('submit', function(e) {
            ...
            .then(function(data) {
                ...
                if (data && data.success) {
                    ...
                    if (addCustomerRegionSelect) {
                        addCustomerRegionSelect.appendChild(newOption);
                        addCustomerRegionSelect.value = data.region.id;
                    }
                    
                    if (editCustomerRegionSelect) {
                        var editOption = newOption.cloneNode(true);
                        editCustomerRegionSelect.appendChild(editOption);
                    }
...
<div class="modal fade d-none d-md-block" id="addRegionFromCustomerModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form id="addRegionFromCustomerForm">
                <div class="modal-body">
                    <div class="mb-3">
                    ...
                    <div id="addRegionMessage" class="alert d-none"></div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">إلغاء</button>
                    <button type="submit" class="btn btn-primary" id="addRegionSubmitBtn">
                        <span class="spinner-border spinner-border-sm d-none me-2" id="addRegionSpinner"></span>
                        إضافة
                    </button>
                </div>
            </form>
...
<?php if (in_array($currentRole, ['manager', 'developer', 'accountant', 'sales'], true)): ?>
<?php
// جلب قائمة المندوبين (فقط للمدراء والمحاسبين والمطورين - فقط في قسم عملاء المندوبين)
$salesRepsList = [];
$isCompanySection = ($section === 'company');
if (!$isSalesUser && !$isCompanySection && in_array($currentRole, ['manager', 'developer', 'accountant'], true)) {
    try {
