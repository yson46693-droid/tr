        ...
        if ($action === 'add_honey') {
            ...
            if ($supplierId <= 0) {
                ...
                $details = sprintf(
                    'إضافة %s (%s) - الحالة: %s',
                    $varietyLabel,
                    $materialLabel !== '' ? $materialLabel : $honeyVariety,
        ...
        elseif ($action === 'record_damage') {
            ...
            if (!in_array($materialCategory, $validCategories, true)) {
                ...
                try {
                    ...
                    if (empty($error) && !ensureRawMaterialDamageLogsTable()) {
                        $error = 'تعذر تجهيز سجل تلفيات المواد الخام.';
                    }
            ...
            try {
                ...
                if ($supplierId <= 0) {
                    ...
                    if ($existing) {
                        ...
                        $insertResult = $db->execute("INSERT INTO olive_oil_stock (supplier_id, quantity) VALUES (?, ?)", [$supplierId, $quantity]);
                        if (!empty($insertResult['insert_id'])) {
                            $supplyStockId = (int)$insertResult['insert_id'];
                        }
                    }
                    ...
                    $supplierName = $supplierRow['name'] ?? null;
                    recordProductionSupplyLog([
                        'material_category' => 'olive_oil',
                        'material_label' => 'زيت زيتون',
                        'stock_source' => 'olive_oil_stock',
                        'stock_id' => $supplyStockId,
                        'supplier_id' => $supplierId,
                        'supplier_name' => $supplierName,
                        'quantity' => $quantity,
        ...
        elseif ($action === 'add_beeswax') {
            ...
            if ($supplierId <= 0) {
                ...
                if ($existing) {
                    ...
                    $insertResult = $db->execute("INSERT INTO beeswax_stock (supplier_id, weight) VALUES (?, ?)", [$supplierId, $weight]);
                    if (!empty($insertResult['insert_id'])) {
                        $supplyStockId = (int)$insertResult['insert_id'];
                    }
                }
                ...
                $supplierName = $supplierRow['name'] ?? null;
                recordProductionSupplyLog([
                    'material_category' => 'beeswax',
                    'material_label' => 'شمع عسل',
                    'stock_source' => 'beeswax_stock',
                    'stock_id' => $supplyStockId,
                    'supplier_id' => $supplierId,
                    'supplier_name' => $supplierName,
                    'quantity' => $weight,
                    'unit' => 'كجم',
                    'details' => 'إضافة شمع عسل خام إلى المخزون',
                    'recorded_by' => $currentUser['id'] ?? null,
                ]);
        ...
        elseif ($action === 'add_derivative') {
            ...
            if ($supplierId <= 0) {
                ...
                if ($existing) {
                    ...
                    $insertResult = $db->execute("INSERT INTO derivatives_stock (supplier_id, derivative_type, weight) VALUES (?, ?, ?)", [$supplierId, $derivativeType, $weight]);
                    if (!empty($insertResult['insert_id'])) {
                        $supplyStockId = (int)$insertResult['insert_id'];
                    }
                }
                ...
                recordProductionSupplyLog([
                    ...
                    'quantity' => $weight,
                    'unit' => 'كجم',
                    'details' => 'إضافة مشتق جديد إلى المخزون',
                    'recorded_by' => $currentUser['id'] ?? null,
                ]);
        ...
        elseif ($action === 'add_single_nuts') {
            ...
            if ($supplierId <= 0) {
                ...
                if ($existing) {
                    ...
                    $insertResult = $db->execute("INSERT INTO nuts_stock (supplier_id, nut_type, quantity, notes) VALUES (?, ?, ?, ?)", [$supplierId, $nutType, $quantity, $notes ?: null]);
                    if (!empty($insertResult['insert_id'])) {
                        $supplyStockId = (int)$insertResult['insert_id'];
                    }
                }
                ...
                $supplierRow = $db->queryOne("SELECT name FROM suppliers WHERE id = ? LIMIT 1", [$supplierId]);
                $supplierName = $supplierRow['name'] ?? null;
                $detailsParts = ['إضافة مكسرات خام'];
                if ($notes !== '') {
                    $detailsParts[] = 'ملاحظات: ' . mb_substr($notes, 0, 120, 'UTF-8');
                }
                recordProductionSupplyLog([
                    'material_category' => 'nuts',
                    'material_label' => 'مكسرات - ' . $nutType,
                    'stock_source' => 'nuts_stock',
                    'stock_id' => $supplyStockId,
                    'supplier_id' => $supplierId,
                    'supplier_name' => $supplierName,
                    'quantity' => $quantity,
...
// الكود القديم تم حذفه لمنع أخطاء JavaScript
