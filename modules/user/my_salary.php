...
if (!function_exists('calculateTotalSalaryWithCollections')) {
     ...
     */
    function calculateTotalSalaryWithCollections($salaryRecord, $userId, $month, $year, $role, $hourlyRateFromUser = null) {
        global $bonusColumnName;
        // حساب الراتب الأساسي دائماً من الساعات × سعر الساعة - نستخدم سعر الساعة من جدول users
        require_once __DIR__ . '/../../includes/salary_calculator.php';
        $hourlyRate = $hourlyRateFromUser !== null ? cleanFinancialValue($hourlyRateFromUser) : cleanFinancialValue($salaryRecord['hourly_rate'] ?? 0);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'request_advance') {
    ...
    // يجب أن يكون نفس الراتب الإجمالي المعروض في جدول التفاصيل
    require_once __DIR__ . '/../../includes/salary_calculator.php';
    
    // استخدام سعر الساعة من جدول users دائماً
    $hourlyRate = cleanFinancialValue($currentUser['hourly_rate'] ?? 0);
    // استخدام اسم العمود الصحيح (bonus أو bonuses)
    $bonus = cleanFinancialValue($salaryRecord[$bonusColumnName] ?? $salaryRecord['bonus'] ?? $salaryRecord['bonuses'] ?? 0);
    $deductions = cleanFinancialValue($salaryRecord['deductions'] ?? 0);
    ...
    if ($currentUser['role'] === 'sales') {
        // استخدام collections_bonus المحفوظ في قاعدة البيانات (يتضمن مكافآت من pos.php)
        $savedCollectionsBonus = cleanFinancialValue($salaryRecord['collections_bonus'] ?? 0);
        if ($savedCollectionsBonus > 0) {
            $collectionsBonus = $savedCollectionsBonus;
        } else {
            // إذا لم تكن هناك قيمة محفوظة، احسبها من جدول collections
            $collectionsAmount = calculateSalesCollections($currentUser['id'], $month, $year);
            $collectionsBonus = round($collectionsAmount * 0.02, 2);
        }
    }
    
    // حساب الراتب الإجمالي (قبل الخصومات) = الراتب الأساسي + المكافآت + نسبة التحصيلات
    // هذا هو نفس الراتب الإجمالي المعروض في جدول التفاصيل (162.10)
    if ($currentUser['role'] === 'sales') {
        $grossSalary = round($baseAmount + $bonus + $collectionsBonus, 2);
    } else {
        $grossSalary = round($baseAmount + $bonus, 2);
    }
...
if ($salaryData['exists']) {
    ...
    // حساب نسبة التحصيلات للمندوبين
    $collectionsBonusForAdvance = 0;
    if ($currentUser['role'] === 'sales') {
        $salaryCalculation = calculateTotalSalaryWithCollections($currentSalary, $currentUser['id'], $selectedMonth, $selectedYear, $currentUser['role'], $currentUser['hourly_rate'] ?? 0);
        $collectionsBonusForAdvance = $salaryCalculation['collections_bonus'] ?? 0;
    }
    
    // حساب الراتب الإجمالي (قبل الخصومات)
    if ($currentUser['role'] === 'sales') {
        $grossSalaryForAdvance = round($baseAmountForAdvance + $bonusForAdvance + $collectionsBonusForAdvance, 2);
    } else {
        $grossSalaryForAdvance = round($baseAmountForAdvance + $bonusForAdvance, 2);
    }
    
    // حساب الراتب الصافي (بعد الخصومات)
    $netSalaryForAdvance = max(0, round($grossSalaryForAdvance - $deductionsForAdvance, 2));
    
    // حساب الحد الأقصى للسلفة بناءً على نصف الراتب الصافي (بعد الخصومات)
    $maxAdvance = cleanFinancialValue($netSalaryForAdvance * 0.5);
    ...
    // حساب الراتب الأساسي دائماً من الساعات × سعر الساعة (حقل ثابت لا يتأثر بالتسويات)
    require_once __DIR__ . '/../../includes/salary_calculator.php';
    $hourlyRateForAdvance = cleanFinancialValue($currentUser['hourly_rate'] ?? 0);
...
if ($currentSalary) {
    ...
    // حساب الراتب الأساسي دائماً من الساعات × سعر الساعة (حقل ثابت لا يتأثر بالتسويات)
    require_once __DIR__ . '/../../includes/salary_calculator.php';
    $hourlyRateForStats = cleanFinancialValue($currentUser['hourly_rate'] ?? 0);
    ...
    $netSalaryForStats = max(0, round($grossSalaryForStats - $deductionsForStats, 2));
    $maxAdvance = cleanFinancialValue($netSalaryForStats * 0.5);
} else {
